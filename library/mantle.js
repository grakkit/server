...
const ArrayList = type_1.type('java.util.ArrayList');
const Block = type_1.type('org.bukkit.block.Block');
const BlockStateMeta = type_1.type('org.bukkit.inventory.meta.BlockStateMeta');
const BoundingBox = type_1.type('org.bukkit.util.BoundingBox');
const CommandSender = type_1.type('org.bukkit.command.CommandSender');
const Entity = type_1.type('org.bukkit.entity.Entity');
const EntityType = type_1.type('org.bukkit.entity.EntityType');
const ItemFlag = type_1.type('org.bukkit.inventory.ItemFlag');
const ItemStack = type_1.type('org.bukkit.inventory.ItemStack');
const LivingEntity = type_1.type('org.bukkit.entity.LivingEntity');
const Location = type_1.type('org.bukkit.Location');
const Material = type_1.type('org.bukkit.Material');
const NamespacedKey = type_1.type('org.bukkit.NamespacedKey');
const OfflinePlayer = type_1.type('org.bukkit.OfflinePlayer');
const PersistentDataHolder = type_1.type('org.bukkit.persistence.PersistentDataHolder');
const PersistentDataType = type_1.type('org.bukkit.persistence.PersistentDataType');
const SpawnReason = type_1.type('org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason');
const UUID = type_1.type('java.util.UUID');
const Vector = type_1.type('org.bukkit.util.Vector');
const server = type_1.type('org.bukkit.Bukkit').getServer();
const NMS = `net.minecraft.server.${server.getClass().getCanonicalName().split('.')[3]}`;
const NBTTagByte = type_1.type(`${NMS}.NBTTagByte`);
const NBTTagByteArray = type_1.type(`${NMS}.NBTTagByteArray`);
const NBTTagCompound = type_1.type(`${NMS}.NBTTagCompound`);
const NBTTagDouble = type_1.type(`${NMS}.NBTTagDouble`);
const NBTTagFloat = type_1.type(`${NMS}.NBTTagFloat`);
const NBTTagInt = type_1.type(`${NMS}.NBTTagInt`);
const NBTTagIntArray = type_1.type(`${NMS}.NBTTagIntArray`);
const NBTTagList = type_1.type(`${NMS}.NBTTagList`);
const NBTTagLong = type_1.type(`${NMS}.NBTTagLong`);
const NBTTagLongArray = type_1.type(`${NMS}.NBTTagLongArray`);
const NBTTagShort = type_1.type(`${NMS}.NBTTagShort`);
const NBTTagString = type_1.type(`${NMS}.NBTTagString`);
...
function data(arg1, arg2 = 'default') {
    ...
    if (typeof arg2 !== 'string') {
        ...
        return index_1.core.data(`${store}/block/${thing.getWorld().getUID().toString()}/${thing.getBlockKey().toString(16)}`);
    }
    else if (thing instanceof OfflinePlayer) {
        return index_1.core.data(`${store}/player/${thing.getUniqueId().toString()}`);
    }
    else if (thing instanceof Entity) {
        return index_1.core.data(`${store}/entity/${thing.getUniqueId().toString()}`);
    }
        ...
        if (meta instanceof PersistentDataHolder) {
            ...
            return index_1.core.data(`${store}/itemStack/${container.get(key, type)}`);
        }
    }
}
exports.data = data;
...
function select(arg1 = '@e', arg2 = server.getConsoleSender()) {
...
event_1.event('org.bukkit.event.inventory.InventoryClickEvent', (event) => {
